...
- 保留 `data/` 中已公开的案例原始资料及其目录结构，见[数据说明](data/README.zh-CN.md)。其他使用者的私人输入仍不提交，除非明确授权并经过公开审查。不提交私人输入、输出模型、逐帧渲染、虚拟环境或访问凭据。预览图放 `docs/assets/`；经审查的公开成果放 `artifacts/`，同时更新 `.gitignore` 的文件清单，并使用 Git LFS。
