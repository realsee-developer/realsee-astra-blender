...
这里整理了一次实际项目的原始资料、提示词、上手教程、空间建模代码和成果预览。重点是墙、地面、顶面、门窗和区域连接，家具与小物件作为补充。
...
2. 通过 Git LFS 将[案例原始资料](data/README.zh-CN.md)下载到 `data/`，或使用自己的 Realsee 模型与贴图、点云、全景图、CAD 等资料。保留原始目录结构，不覆盖已公开的案例文件。
...
案例包含 396 个原始文件，共 6,079,895,800 字节（约 5.66 GiB），不含 `.DS_Store`。安装 [Git LFS](https://git-lfs.com/) 后，可以仅下载原始资料：
...
文件清单与校验方法见[数据说明](data/README.zh-CN.md)。原始资料按原样公开，包含现场二维码；现有公开模型与视频仍为先前经过审查、脱敏的副本。原始素材的权利与代码许可分开，详见[数据与许可](docs/data-and-license.md)。
...
| [data/](data/README.zh-CN.md) | 通过 Git LFS 提供案例原始资料；额外的私人输入仍不提交 |
| `research/`、`output/` | 本地分析与生成文件，Git 默认忽略 |
...
整套原始扫描及全景数据也通过 Git LFS 提供。历史测量结果、研究记录与中间检查点仍留在本地，因此精选脚本并不是开箱即用的完整复现流程。代码、提示词和原创文档采用 [MIT](LICENSE)，原始资料与案例素材的权利单独说明，见[数据与许可](docs/data-and-license.md)。
