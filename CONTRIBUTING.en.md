...
Build and preview from the repository root (macOS/Linux):
...
Generated content goes into `output/site/`, which is rebuilt on each run. The builder copies only the three public previews and stylesheet. Site checks cover local links, heading anchors, and language switches. Raw inputs and large models are not part of the website package.
